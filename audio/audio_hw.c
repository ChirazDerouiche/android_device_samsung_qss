                        ...
                        bool on, bool enforce, bool reset, int reset_state)
{
    ...
    for (i = cfgs_nr - 1; i >= 0; i--) {
        ...
        if (!cfg->ctl) {
            ...
            if (!cfg->ctl) {
                LOGE("unable to get mixer ctl '%s'", cfg->name);
                continue;
            }
        }
        ...
        if (cfg->strval) {
            if (mixer_ctl_get_type(cfg->ctl) != MIXER_CTL_TYPE_ENUM) {
                LOGE("mixer ctl '%s' is not enum", cfg->name);
                continue;
            }
            ...
            if (cfg->strval) {
                LOGE("mixer ctl '%s' invalid enum '%s'", cfg->name, cfg->strval);
                continue;
            }
        }

        LOGD("apply mixer '%s' state=%d(%d) devices=%d(%d) devices_masked=%d", cfg->name, state, cfg->state,
    ...
    while(pnext) {
        LOGD("apply defered mixer '%s' state=%d", pnext->name, pnext->state);
...
static int out_set_parameters(struct audio_stream *stream, const char *kvpairs)
{
    ...
    LOGD("%s: kvpairs %s", __func__, kvpairs);
    ...
    if (ret >= 0) {
        ...
        LOGD("%s: set route to=%x cur=%x", __func__, val, (adev->devices & AUDIO_DEVICE_OUT_ALL));
...
static int in_set_parameters(struct audio_stream *stream, const char *kvpairs)
{
    ...
    if (ret >= 0) {
        val = atoi(value);
        pthread_mutex_lock(&adev->lock);
        LOGD("%s: set route to=%x cur=%x", __func__, val, (adev->devices & AUDIO_DEVICE_IN_ALL));
                                   ...
                                   struct resampler_buffer* buffer)
{
    ...
    if (in->frames_in == 0) {
        ...
        if (in->read_status != 0) {
            LOGE("in_get_next_buffer() pcm_read error %d", in->read_status);
            buffer->raw = NULL;
            buffer->frame_count = 0;
...
static int adev_set_voice_volume(struct audio_hw_device *dev, float volume)
{
    ...
    LOGD("%s: route=%x volume=%f", __func__, adev->devices, volume);
                                  ...
                                  struct audio_stream_in **stream_in)
{
    ...
    LOGD("%s: channel_count=%d raw=%x", __func__, channel_count, *channels);
                     ...
                     hw_device_t** device)
{
    ...
    if (!adev->mixer) {
        LOGE("cannot open mixer ctl on card %d", CARD_QSS_DEFAULT);
        ret = -ENODEV;
        goto err_free;
    }
    ...
    if (ret) {
        LOGE("cannot open ril interface err=%d", ret);
        goto err_mixer;
    }
